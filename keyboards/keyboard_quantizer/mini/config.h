...
// mini.c内の、アナログジョイスティックをマウスカーソルに変換する独自コード
// (matrix_init_kb/matrix_scan_kbから呼ばれるanalog_joystick_init/task)を
// 有効化する、本プロジェクト独自の設定フラグ。
// Enables the custom analog-joystick-to-mouse-cursor code in mini.c
...
// used by this code (see analog_joystick_read_10bit in mini.c). Only a
...
// このコードが使う10bit換算ADCスケール(mini.cのanalog_joystick_read_10bit
...
// keymap generated by dynamic_keymap_reset() in mini.c. Range: 0 to
// MATRIX_ROWS-1, must be less than MATRIX_MSBTN_ROW.
// mini.cのdynamic_keymap_reset()が生成するデフォルトキーマップで、
...
// オンボードWS2812 LED(Waveshare RP2040-Zero)のGPIOピン番号。mini.c内で
...
// directly via ws2812_setleds() in mini.c for debug input indication.
